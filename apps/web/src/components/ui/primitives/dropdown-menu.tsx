...
import { ComponentProps, createContext, MouseEvent, ReactElement, useContext, useState } from "react"
import { cn } from "~/utils/ui"

/**
 * @remarks Adapted to allow manual control of the menu via context.
 */
function DropdownMenu({ onOpenChange, open, ...props }: ComponentProps<typeof DropdownMenuPrimitive.Root>) {
    const [isOpenState, setIsOpenState] = useState(false)

    const isOpenStateControlled = open !== undefined
    const isOpen = isOpenStateControlled ? open : isOpenState

    const handleOpenChange = (newState: boolean) => {
        if (isOpenStateControlled === false) setIsOpenState(newState)
    ...
    return (
        <DropdownMenuContext.Provider value={{ open: handleOpen, close: handleClose }}>
            <DropdownMenuPrimitive.Root data-slot="dropdown-menu" open={isOpen} onOpenChange={handleOpenChange} {...props} />
...
/**
 * @deprecated Use `FlashDropdownMenuItem` instead for better performance. This component has performance issues when hovering over multiple items due to Radix UI's focus management overhead.
 */
...
export function useDropdownMenu() {
    ...
    if (!dropdownMenu) throw new Error("`useDropdownMenu` must be used within a `DropdownMenuProvider`.")

    return dropdownMenu
}

function FlashDropdownMenuItem({ children, className, ...props }: { children: ReactElement<{ onClick?: (event: MouseEvent) => void }> } & ComponentProps<typeof Slot>) {
    const dropdownMenu = useDropdownMenu()

    return (
        <Slot onClick={dropdownMenu.close} className={className} {...props}>
            {children}
        </Slot>
    )
}

export { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuGroup, DropdownMenuItem, DropdownMenuLabel, DropdownMenuPortal, DropdownMenuRadioGroup, DropdownMenuRadioItem, DropdownMenuSeparator, DropdownMenuShortcut, DropdownMenuSub, DropdownMenuSubContent, DropdownMenuSubTrigger, DropdownMenuTrigger, FlashDropdownMenuItem }
