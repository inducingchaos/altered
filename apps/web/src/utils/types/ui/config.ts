...
import type { FlattenObjectToArrayLiteralUnion, TupleToObject } from ".."

export type InternalComponentConfig<
    DataStates extends readonly string[] = [],
    UIStates extends readonly string[] = []
> = {
    State: Record<DataStates[number], UIStates[number][] | UIStates[number]>
}

export type DiscriminateStateConfig<
    Config extends Record<string, string | string[]>
> = TupleToObject<FlattenObjectToArrayLiteralUnion<Config>, ["data", "ui"]>

export type InternalComponentProps<
    _StateConfig extends InternalComponentConfig["State"]
> = { _config?: { state?: DiscriminateStateConfig<_StateConfig> } }
