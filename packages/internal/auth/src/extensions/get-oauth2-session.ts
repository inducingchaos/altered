...
export async function getOAuth2Session({
    ...
    if (!accessToken)
        return {
            error: {
                code: "UNAUTHORIZED",
                message: "The provided authorization token does not exist.",
                cause: { authorization: headers.get("authorization") }
            }
        }
    ...
    if (!trustedClientIds.includes(accessToken.clientId)) {
        const oauthApplication = await db.query.oauthApplications.findFirst({
        ...
        if (!oauthApplication)
            return {
                error: {
                    code: "UNAUTHORIZED",
                    message:
                        "The provided authorization token is not associated with a trusted client.",
                    cause: { authorization: headers.get("authorization") }
                }
            }
    }
    ...
    if (!user)
        return {
            error: {
                code: "UNAUTHORIZED",
                message:
                    "The provided authorization token is not associated with a known user.",
                cause: { authorization: headers.get("authorization") }
            }
        }
