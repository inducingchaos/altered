 ...
 */
export type AmbiguousSessionResult =
    | { data: AuthContext; error?: never }
    | {
          data?: never
          error: {
 ...
 */
export async function getAmbiguousSession({
    headers
}: {
    headers: Headers
    ...
    if (headers.get("authorization")) {
        if (headers.get("x-client-id") === "altered-launcher")
            return getOAuth2Session({ headers })
    ...
    if (!(headers.get("cookie") || headers.get("authorization")))
        return {
            error: {
                code: "BAD_REQUEST",
                message: "No authentication headers were provided.",
                cause: {
    ...
    return {
        error: {
            code: "UNAUTHORIZED",
            message: "No authentication session could be found.",
            cause: {
                authorization: headers.get("authorization"),
                cookie: headers.get("cookie")
            }
