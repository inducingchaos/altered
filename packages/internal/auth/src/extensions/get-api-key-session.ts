 ...
 */
export async function getApiKeySession({ headers }: { headers: Headers }): Promise<AmbiguousSessionResult> {
    const apiKeyValue = headers.get("authorization")?.replace("Bearer ", "")
    if (!apiKeyValue) return { error: { code: "BAD_REQUEST", message: "No API key was provided.", cause: { authorization: headers.get("authorization") } } }
    ...
    if (!user || !apiKey) return { error: { code: "UNAUTHORIZED", message: "The provided API key does not exist.", cause: { authorization: headers.get("authorization") } } }
    ...
    return {
        data: {
            user,
            session: { ...apiKey, token: apiKey.value, expiresAt: tempExpiresAt }
        }
    }
}
